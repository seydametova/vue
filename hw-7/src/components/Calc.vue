<template>
  <div class="calc">
    <div class="main">
        <input v-model="op1" type="number" name="operand1"/>
        <input v-model="op2" type="number" name="operand2"/>
        = {{ result }}
    </div>
    ...
    <div class="showElements"  v-show="show">
      <div class="buttonNumber">
        <button v-for="operand1 in operands1"
                :key="operand1"
                :name="'keyboard' + operand1"
                :title="operand1"
                @click="addChar(operand1)"
                class="numbers">
          {{ operand1 }}
        </button>
        <button name="keyboard-backspace" @click="removeChar()">
          &#8592;
        </button>
      </div>
      <div class="operands">
        <input type="radio" id="op1" name="operand-selector" value="op1" v-model="selectedOperand">
        <label for="op1">Operand 1</label>
        <input type="radio" id="op2" name="operand-selector" value="op2" v-model="selectedOperand">
...
export default {
  ...
  methods: {
    ...
    div () {
      ...
      this.result = Math.round(op1 / op2 * 100) / 100
    },
    multi () {
      const { op1, op2 } = this
      this.result = op1 * op2
